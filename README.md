...
print(admin) #output: {'username': 'mewhrzad', 'is_sudo': True, 'telegram_id': 123456789, 'discord_webhook': None}
```
### Create Admin
```python
...
result = await panel.modify_admin(username=target_admin,token=mytoken,data=info)
print(result) #output: success
```
### Remove Admin
```python
target_admin = "test"
result = await panel.remove_admin(username=target_admin,token=mytoken)
print(result) #output: success
```
### Get All Admins
```python
